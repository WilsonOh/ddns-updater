...
func main() {
	...
	// Initialize metrics
	metricsConfig, err := metrics.LoadConfigFromEnv()
	if err != nil {
		slog.Error("failed to load metrics config", slog.Any("error", err))
		os.Exit(1)
	}
	...
	defer func(start time.Time) {
		metricsRecorder.RecordRun(runStatus, time.Since(start), err)
	}(time.Now())
	...
	formattedResp, _ := json.Marshal(res)
